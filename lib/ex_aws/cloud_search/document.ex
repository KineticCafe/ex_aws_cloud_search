defmodule ExAws.CloudSearch.Document do
  ...
  """
  @spec add(map | struct) :: request | no_return
  def add(%_mod{} = document), do: add(add_normalize(document))

  def add(%{id: _} = document), do: add(add_normalize(document))

  def add(%{"id" => _} = document), do: add(add_normalize(document))

  def add(documents) when is_map(documents), do: add(Enum.into(documents, []))

  @spec add({String.t(), map}) :: request | no_return
  def add({_id, _fields} = document), do: add([document])

  @spec add(list(map | struct | {term, fields})) :: request | no_return
  def add([]), do: raise(ExAws.Error, "Must provide documents to index.")
  ...
  Removes the provided document(s) by ID from the index.
      ...
      remove([3])
  """
  @spec remove(map | struct | term) :: request | no_return
  def remove(%_mod{} = document), do: remove(remove_normalize(document))

  def remove(%{id: _} = document), do: remove(remove_normalize(document))

  def remove(%{"id" => _} = document), do: remove(remove_normalize(document))

  @spec remove(list) :: request | no_return
  def remove([]), do: raise(ExAws.Error, "Must provide documents to remove.")
  ...
  def remove(document), do: remove(remove_normalize(document))

  defp add_normalize(%_mod{id: _} = document), do: add_normalize(Map.from_struct(document))
  ...
  defp add_normalize(%{id: id} = document), do: add_normalize({id, Map.delete(document, :id)})

  defp add_normalize(%{"id" => id} = document),
    do: add_normalize({id, Map.delete(document, "id")})
  ...
  defp remove_normalize(%_mod{id: id}), do: [id]

  defp remove_normalize(%{id: id}), do: [id]

  defp remove_normalize(%{"id" => id}), do: [id]

  defp remove_normalize(%mod{}),
    do:
      raise(
        ExAws.Error,
  ...
  defp remove_normalize(id), do: List.wrap(id)
  ...
  defp add_normalize_field({key, %Date{} = value}),
    do: {key, "#{Date.to_iso8601(value)}T00:00:00Z"}

  defp add_normalize_field({key, %Time{}}) do
    raise ExAws.Error, "Cannot add a Time as a document field for #{key}."
  end

  defp add_normalize_field({key, %DateTime{} = value}), do: {key, DateTime.to_iso8601(value)}

  defp add_normalize_field({key, []}), do: {key, nil}

  defp add_normalize_field({key, ""}), do: {key, nil}

  defp add_normalize_field(pair), do: pair
